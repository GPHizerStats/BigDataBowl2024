# # Special Thanks to Marschall Furman for the development of the gg_field function
# # Twitter: @mlfurman3
# # Github: github.com/mlfurman3/gg_field
...
gg_field <- function(yardmin=0, yardmax=120, buffer=5, direction="horiz",
  ...
  p <- list(
    ...
    annotate("rect", xmin=xmin, xmax=xmax, ymin=ymin-buffer, ymax=ymax+buffer, 
             fill=field_color, alpha=0.5),
    
    ## add end zones
    annotate("rect", xmin=xmin, xmax=xmin+10, ymin=ymin, ymax=ymax, fill=endzone_color, alpha=0.5),
    annotate("rect", xmin=xmax-10, xmax=xmax, ymin=ymin, ymax=ymax, fill=endzone_color, alpha=0.5),
    
    ## add yardlines every 5 yards
    annotate("segment", x=yd_lines, y=ymin, xend=yd_lines, yend=ymax,
             col=line_color, alpha=0.5),
    ...
    annotate("segment",x=c(0,10,60,110,120), y=ymin, xend=c(0,10,60,110,120), yend=ymax,
             lwd=1.3, col=line_color, alpha=0.5),
    ...
    ## 0
    annotate("text",x=seq(20,100,by=10) + num_adj[2], y=ymin+11, label=0, angle=angle_vec[1],
             col=line_color, size=num_size, alpha=0.5),
    
    ## 1
    annotate("text",label=4,x=c(20,100) + num_adj[1], y=ymin+11, angle=angle_vec[1],
             colour=line_color, size=num_size, alpha=0.5),
    ## 2
    annotate("text",label=3,x=c(30,90) + num_adj[1], y=ymin+11, angle=angle_vec[1],
             colour=line_color, size=num_size, alpha=0.5),
    ## 3
    annotate("text",label=2,x=c(40,80) + num_adj[1], y=ymin+11, angle=angle_vec[1],
             colour=line_color, size=num_size, alpha=0.5),
    ## 4
    annotate("text",label=1,x=c(50,70) + num_adj[1], y=ymin+11, angle=angle_vec[1],
             colour=line_color, size=num_size, alpha=0.5),
    ## 5
    annotate("text",label=0,x=60 + num_adj[1], y=ymin+11, angle=angle_vec[1],
             colour=line_color, size=num_size, alpha=0.5),
    ...
    ## 0
    annotate("text",x=seq(20,100,by=10) + num_adj[1], y=ymax-11, angle=angle_vec[2],
             label=0, col=line_color, size=num_size, alpha=0.5),
    ## 1
    annotate("text",label=4,x=c(20,100) + num_adj[2], y=ymax-11, angle=angle_vec[2],
             colour=line_color, size=num_size, alpha=0.5),
    ## 2
    annotate("text",label=3,x=c(30,90) + num_adj[2], y=ymax-11, angle=angle_vec[2],
             colour=line_color, size=num_size, alpha=0.5),
    ## 3
    annotate("text",label=2,x=c(40,80) + num_adj[2], y=ymax-11, angle=angle_vec[2],
             colour=line_color, size=num_size, alpha=0.5),
    ## 4
    annotate("text",label=1,x=c(50,70) + num_adj[2], y=ymax-11, angle=angle_vec[2],
             colour=line_color, size=num_size, alpha=0.5),
    ## 5
    annotate("text",label=0,x=60 + num_adj[2], y=ymax-11, angle=angle_vec[2],
             colour=line_color, size=num_size, alpha=0.5),
    ...
    annotate("segment", x=yd_hash, y=ymax - hash_dist - 0.5, 
             xend=yd_hash, yend=ymax - hash_dist + 0.5,color=line_color, alpha=0.5),
    
    ## add hash marks - sidelines
    annotate("segment", x=yd_hash, y=ymax, xend=yd_hash, yend=ymax-1, color=line_color, alpha=0.5),
    annotate("segment", x=yd_hash, y=ymin, xend=yd_hash, yend=ymin+1, color=line_color, alpha=0.5),
    
    ## add conversion lines at 2-yard line
    annotate("segment",x=12, y=(ymax-1)/2, xend=12, yend=(ymax+1)/2, color=line_color, alpha=0.5),
    annotate("segment",x=108, y=(ymax-1)/2, xend=108, yend=(ymax+1)/2, color=line_color, alpha=0.5),
    
    ## cover up lines outside of field with sideline_color
    annotate("rect", xmin=0, xmax=xmax, ymin=ymax, ymax=ymax+buffer, fill=sideline_color, alpha=0.5),
    annotate("rect",xmin=0, xmax=xmax, ymin=ymin-buffer, ymax=ymin, fill=sideline_color, alpha=0.5),
